...
import { getLocalBookings, saveLocalBooking, deleteLocalBooking, getVehicles } from '../utils/storage';
...
const LocalBooking = () => {
  ...
  const [formData, setFormData] = useState({
    id: '', customerName: '', pickup: '', drop: '', 
    vehicleNumber: '', date: '', fare: '', paymentStatus: 'Unpaid'
  });
  ...
  const loadData = async () => {
    setBookings(await getLocalBookings());
  ...
  const handleOpenModal = (b = null) => {
    if (b) {
      ...
      setFormData({
        ...
        date: new Date().toISOString().split('T')[0], fare: '', paymentStatus: 'Unpaid'
      });
    }
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await saveLocalBooking(formData);
    loadData();
    setShowModal(false);
  };
  ...
  if(filterSearch) {
    filteredBookings = filteredBookings.filter(b => 
      b.customerName.toLowerCase().includes(filterSearch.toLowerCase()) ||
      b.vehicleNumber.toLowerCase().includes(filterSearch.toLowerCase())
    );
  }
  if(filterDate) {
    ...
    <>
      <div className="animate-in pb-5">
        ...
        <div className="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-5 gap-4">
          <div>
            <h2 className="fw-800 mb-1" style={{ color: '#1e293b', letterSpacing: '-0.5px', fontSize: 'clamp(1.6rem, 5vw, 2.2rem)' }}>Local Dispatches</h2>
            <p className="text-secondary fw-500 mb-0">Manage daily local transport operations</p>
          </div>
        ...
        <div className="card border-0 shadow-sm overflow-hidden" style={{ borderRadius: '24px' }}>
          <div className="card-header bg-white border-bottom p-4 px-3 px-md-5 d-flex align-items-center justify-content-between flex-wrap gap-3">
              <div className="d-flex align-items-center gap-3">
                  ...
                  <h5 className="fw-800 mb-0" style={{ color: '#1e293b', fontSize: '1.1rem' }}>Dispatch Activity Log</h5>
              </div>
            ...
            <table className="table table-mobile-cards align-middle mb-0">
              <thead className="bg-light">
                <tr style={{ height: '50px' }}>
                  <th className="px-4 py-3 text-secondary fw-800 small" style={{ letterSpacing: '0.05em' }}>DATE & VEHICLE</th>
              ...
              <tbody>
                {currentRecords.length === 0 ? (
                  <tr><td colSpan="6" className="text-center py-5 text-muted fw-700">NO LOCAL DISPATCHES FOUND</td></tr>
                ) : (
                  currentRecords.map((b) => (
                    <tr key={b.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                      <td data-label="DATE & VEHICLE" className="px-4 py-4">
          ...
          <div className="modal-dialog modal-lg m-0 m-md-auto" style={{ maxWidth: '650px' }}>
            <form onSubmit={handleSubmit} className="modal-content shadow-2xl border-0">
              <div className="modal-header border-0 d-flex justify-content-between align-items-center">
                <h4 className="modal-title fw-800 text-dark" style={{ letterSpacing: '-0.5px' }}>Local Dispatch Entry</h4>
              ...
              <div className="modal-body">
                  <div className="row g-4">
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-800 small text-secondary opacity-80 mb-2" style={{ fontSize: '10.5px', letterSpacing: '0.5px' }}>CUSTOMER NAME</label>
                      <input type="text" className="form-control rounded-lg shadow-none" value={formData.customerName} onChange={e => setFormData({...formData, customerName: e.target.value})} placeholder="e.g. Rahul Sharma" required />
                    </div>
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-800 small text-secondary opacity-80 mb-2" style={{ fontSize: '10.5px', letterSpacing: '0.5px' }}>PICKUP POINT</label>
                      <input type="text" className="form-control rounded-lg shadow-none" value={formData.pickup} onChange={e => setFormData({...formData, pickup: e.target.value})} placeholder="Enter pickup city" required />
                    </div>
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-800 small text-secondary opacity-80 mb-2" style={{ fontSize: '10.5px', letterSpacing: '0.5px' }}>DROP POINT</label>
                    ...
                    <div className="col-12 col-md-6">
                      <label className="form-label fw-800 small text-secondary opacity-80 mb-2" style={{ fontSize: '10.5px', letterSpacing: '0.5px' }}>FARE (₹)</label>
                    ...
                    <div className="col-12">
                      <label className="form-label fw-800 small text-secondary opacity-80 mb-2" style={{ fontSize: '10.5px', letterSpacing: '0.5px' }}>PAYMENT STATUS</label>
                      <div className="d-flex gap-3">
                        {['Unpaid', 'Partial', 'Paid'].map(status => (
                          <button key={status} type="button" className={`btn rounded-pill px-4 py-2 fw-800 fs-7 transition-all ${formData.paymentStatus === status ? 'btn-primary shadow-sm' : 'btn-light'}`} style={{ fontSize: '11px' }} onClick={() => setFormData({...formData, paymentStatus: status})}>
                            {status}
                          </button>
                        ))}
                      </div>
              ...
              <div className="modal-footer-custom">
                ...
                <button type="submit" className="btn btn-primary px-4 py-2 fw-800 shadow-sm border-0" style={{ borderRadius: '10px', background: '#4f46e5' }}>Commit Entry</button>
              </div>
            </form>
          </div>
        </div>
